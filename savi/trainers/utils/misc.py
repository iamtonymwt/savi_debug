...
def load_snapshot(args, model, optimizer, device, path, types):
	print('loading model.')
	
	for ty in types:
		snapshot_path = path[ty]
		if ty == 0:
			payload = torch.load(snapshot_path, map_location=device)
			model.load_state_dict(payload['model'], strict=True)
			print('strictly loaded model.')
		elif ty == 1:
			#encoder
			encoder_payload = torch.load(snapshot_path, map_location=device)
			#new_encoder_payload = {k.replace('module.','encoder.backbone.'):v for k,v in encoder_payload['model'].items()}
			#del new_encoder_payload['encoder.backbone.classifier.classifier.3.weight']
			#del new_encoder_payload['encoder.backbone.classifier.classifier.3.bias']
			new_encoder_payload = {k.replace('module.','encoder.'):v for k,v in encoder_payload['model'].items()}
			model.load_state_dict(new_encoder_payload, strict=False)
			print('weakly loaded model.')
		elif ty == 2:
			#attention
			attention_payload = torch.load(snapshot_path, map_location=device)
			del attention_payload['model']['encoder.pos_emb.pos_embedding']
			del attention_payload['model']['encoder.pos_emb.output_transform.layernorm_module.weight']
			del attention_payload['model']['encoder.pos_emb.output_transform.layernorm_module.bias']
			del attention_payload['model']['encoder.pos_emb.output_transform.model.dense_mlp_0.weight']
			del attention_payload['model']['encoder.pos_emb.project_add_dense.weight']
			del attention_payload['model']['encoder.pos_emb.project_add_dense.bias']
			del attention_payload['model']['decoder.pos_emb.pos_embedding']
			new_attention_payload = {k:v for k,v in attention_payload['model'].items() if k.find('encoder.backbone')==-1}
			model.load_state_dict(new_attention_payload, strict=False)
			print('weakly loaded model.')
...
def viz_seg(vid, gt_mask, pr_mask, output_fn, trunk=None, send_to_wandb=False):
	...
	if(T==1):
		...
		for t in range(T):
			gt_seg = label2rgb(gt_mask[t], recover_image(vid[t]))
			pred_seg = label2rgb(pr_mask[t], recover_image(vid[t]))

			plot_image(ax[0], vid[t], 'frame')
			plot_image(ax[1], gt_seg, 'gt_seg')
			plot_image(ax[2], pred_seg, 'pred_seg')
...
def viz_slots_flow(vid, gt_flow, pr_flow, mask, output_fn, trunk=None, send_to_wandb=False):
	...
	for t in range(T):
		...
		for obj in range(3, n_objs+3):
			plot_image(ax[t, obj], recover_image(slots[t, obj-3]), f'slot {obj-2}')
